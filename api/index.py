...
        # Colorado sering mengambil 'Energi' dari angka tengah (KOP-KEP)
        scores[MB.get(d0[1], '0')] += 15
        scores[TY.get(d0[2], '0')] += 15
...
        # --- LAYER 2: SUM & DIFF FILTER ---
        sum_val = (h + t) % 10
        # Tambahkan filter global untuk pasaran non-spesifik
        if market_name in ['ORLANDO', 'COLORADO', 'MANHATTAN'] or 'OREGON' in market_name:
            if sum_val in [1, 5, 8, 9]: score += 25 
        else:
            # Global Verification untuk pasaran lain (Busan, Jeju, dll)
...
        # AS & KOP Verifikasi Berantai
        # AS diambil dari Mistik Baru AS terakhir atau BBFS terkuat
        as_final = MB.get(d0[0]) if i < 5 else bbfs_list[0]
        # KOP diambil dari Index KEPALA terakhir atau Taysen KOP terakhir
        kop_final = ID.get(d0[2]) if i % 2 == 0 else TY.get(d0[1])
        
        # Validasi Akhir: Jika line 4D terlalu sering muncul, geser angka
        line_4d = f"{as_final}{kop_final}{top2[i]}"
...
    # UBAH: panggil generate_titanium_lines, bukan generate_verified_lines
