...
# ==========================================================
#        [ZONE MODIFIKASI LOGIKA PASARAN TERISOLASI]
# ==========================================================
...
# 3. LOGIKA KHUSUS: SEOUL
def get_seoul_logic(all_res):
    d0 = all_res[0] 
    k_fix, e_fix = TY.get(d0[2], '0'), ML.get(d0[1], '0') 
    line = [k_fix + d0[3], d0[2] + e_fix, ID.get(d0[2]) + d0[3], ML.get(d0[2]) + TY.get(d0[3])]
...
def get_toronto_logic(all_res):
    ...
    return {
        ...
        "kop_kep": ML.get(d0[1], '0') + ML.get(d0[2], '0'),
        "shio": SHIO_MAP.get(shio_idx, "N/A"),
        "macau": f"{SHIO_MAP.get(shio_idx)} - {SHIO_MAP.get((shio_idx + 5) % 12)}",
