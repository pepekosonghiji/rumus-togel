...
SPECIAL_POOLS = {
    ...
    'OREGON 5':'p28049','OREGON 1':'p28046','OREGON 2':'p28047','HONGKONGEVEN':'p2591',
    'OREGON 11':'p28053','JAPANEVEN':'p27501'
}
...
def get_dual_engine_analysis(all_res_data, market_name):
    ...
    # =========================================================================
    # MESIN 1: QUANTUM HYPER-DIMENSION V24.7 [ULTIMATE-SNIPER]
    # =========================================================================
    def run_core_v24_7():
        # --- [INISIALISASI DATA] ---
        ...
        for i, l2 in enumerate(top2):
            try:
                ...
                if kop == l2[0]: kop = ID.get(kop, pos_candidates[(i+3) % 8])
                top3.append(f"{kop}{l2}"); top4.append(f"{as_val}{kop}{l2}")
            except:
                top3.append(f"0{l2}"); top4.append(f"00{l2}")
        ...
        return {
            ...
            'market': market_name, 'last_res': last_p1, 'p2_last': p2_last, 'p3_last': p3_last,
            'am': list(top_digits[:4]), 'ai': list(top_digits[4:7]), 'bbfs': refined_bbfs,
    ...
    # =========================================================================
    # MESIN 2: QUANTUM HYPER-DIMENSION V.26 [HYBRID-POS]
    # =========================================================================
    def run_core_v26_hybrid():
        last_p1 = all_res_data[0][0]  
        p1_list = [d[0] for d in all_res_data] 
        p2_raw = all_res_data[0][1] if len(all_res_data[0]) > 1 else "0000"
        p3_raw = all_res_data[0][2] if len(all_res_data[0]) > 2 else "0000"
        ...
        for d in scores:
            ...
            for pos in range(4):
                if len(all_res_data) > 2:
                    if d == all_res_data[1][0][pos] == all_res_data[2][0][pos]: scores[d] += 120

        top_digits_list = [x[0] for x in sorted(scores.items(), key=lambda x: x[1], reverse=True)]
        top_digits = "".join(top_digits_list[:9])
        am_pool = top_digits_list[:4]
        ai_pool = top_digits_list[4:7]
        ...
        for line, base_score, b_val in pre_top_30:
            ...
            if tail_counts[line[1]] >= 3: final_v_score -= 500
            if line[0] == line[1]:
                if not (last_p1[0] == last_p1[1] or last_p1[2] == last_p1[3]): final_v_score -= 200 
            if final_v_score > 300: 
                final_jitu_2d.append((line, final_v_score))
                tail_counts[line[1]] += 1
        ...
        top2 = [x[0] for x in final_sorted_data]
        top3, top4 = [], []
        front_seeds = ai_pool if len(ai_pool) >= 3 else (ai_pool + am_pool)[:3]
        front_pairs = [''.join(p) for p in itertools.permutations(front_seeds, 2)]

        for i, l2 in enumerate(top2):
            pair = front_pairs[i % len(front_pairs)]
            if pair[1] == l2[0]: pair = front_pairs[(i + 1) % len(front_pairs)]
            as_val, kop_val = pair[0], pair[1]
            top3.append(f"{kop_val}{l2}"); top4.append(f"{as_val}{kop_val}{l2}")
    ...
    # RETURN GABUNGAN UNTUK TEMPLATE OMNI
    return {
        "common": {
            "market": market_name,
            "last": core1_sniper['last_res'],
            "am": core1_sniper['am'],
            "ai": core1_sniper['ai'],
            "am_ai": f"{''.join(core1_sniper['am'])} / {''.join(core1_sniper['ai'])}",
            "shio_off": core1_sniper['shio_off']
        },
        "ori_v24": core1_sniper,
        "hybrid_v26": core2_hybrid
    }
    ...
    if request.method == 'POST':
        ...
        if selected in ALL_POOLS:
            ...
            if res_data and len(res_data) >= 8:
                analysis = get_dual_engine_analysis(res_data, selected)
            else:
